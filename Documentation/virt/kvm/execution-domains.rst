...
guest exit observed by the VMX posted-interrupt helper increments
``root_mode_rejection_count``.  An earlier execution-boundary observation,
including a capsule that has left guest execution or has a pending exit
completion, increments ``boundary_state_rejection_count``.  Both results are
reported as ``EAGAIN`` and userspace may reconcile them only through its own
safe-boundary protocol.  ``target_notification_exit_count`` counts an
