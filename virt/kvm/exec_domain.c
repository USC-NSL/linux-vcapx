...
struct kvm_exec_interrupt_outcome {
	int result;
	u32 status;
	u32 actual_delivery;
	bool retain_halt_armed;
	u64 accepted_tsc;
	u64 publication_started_tsc;
};

static void
kvm_exec_interrupt_outcome_finish(struct kvm_exec_interrupt_outcome *outcome,
				  int result)
{
	...
	switch (result) {
	case 0:
		outcome->status = KVM_EXEC_INTERRUPT_RESULT_ACCEPTED;
		break;
	case -EAGAIN:
		outcome->status = KVM_EXEC_INTERRUPT_RESULT_RETRY_BOUNDARY;
		break;
	case -ESTALE:
	case -ENOENT:
		outcome->status = KVM_EXEC_INTERRUPT_RESULT_STALE;
		break;
	case -EBUSY:
		outcome->status = KVM_EXEC_INTERRUPT_RESULT_BUSY;
		break;
	default:
		outcome->status = KVM_EXEC_INTERRUPT_RESULT_FAILED;
		break;
	}
}
...
struct kvm_exec_executor {
	...
	u32 pending_handoff_entry_timing_slot;
...
static bool kvm_exec_tsc_timing_enabled(struct kvm_exec_executor *executor)
{
	return executor->domain->negotiated_features &
	       KVM_EXEC_FEATURE_TSC_TIMING;
}
...
static struct kvm_exec_tsc_timing_header *
kvm_exec_tsc_timing_header(struct kvm_exec_executor *executor)
{
...
static void kvm_exec_dispatch_init(struct kvm_exec_executor *executor)
{
	...
	if (!kvm_exec_tsc_timing_enabled(executor))
		return;

	timing = kvm_exec_tsc_timing_header(executor);
	timing->abi_version = KVM_EXEC_TSC_TIMING_ABI_VERSION;
	timing->region_size = KVM_EXEC_TSC_TIMING_REGION_SIZE;
	timing->command_timing_offset = KVM_EXEC_TSC_COMMAND_TIMING_OFFSET;
	timing->exit_timing_offset = KVM_EXEC_TSC_EXIT_TIMING_OFFSET;
	timing->command_entries = KVM_EXEC_DISPATCH_RING_ENTRIES;
	timing->exit_entries = KVM_EXEC_DISPATCH_RING_ENTRIES;
	timing->command_entry_size = sizeof(struct kvm_exec_command_timing);
	timing->exit_entry_size = sizeof(struct kvm_exec_exit_timing);
}
...
static void kvm_exec_executor_free(struct kref *kref)
{
	...
	if (executor->dispatch_region)
		free_pages_exact(executor->dispatch_region,
				 kvm_exec_dispatch_region_size(executor));
...
static int kvm_exec_executor_release(struct inode *inode, struct file *file)
{
	...
	mutex_lock(&domain->lock);
	/*
	 * A detached capsule can still carry a snapshotted PIO completion that
	 * is bound to this executor generation.  Closing that executor would
	 * otherwise strand the only authority allowed to apply the response.
				    ...
				    void __user *argp)
{
	...
	if (domain->negotiated_features & KVM_EXEC_FEATURE_DYNAMIC_DISPATCH) {
		executor->dispatch_region =
			alloc_pages_exact(kvm_exec_dispatch_region_size(executor),
...
static struct kvm_exec_command_timing *
kvm_exec_command_timing(struct kvm_exec_executor *executor, u32 slot)
{
	struct kvm_exec_tsc_timing_header *header =
		kvm_exec_tsc_timing_header(executor);

	return (void *)header + KVM_EXEC_TSC_COMMAND_TIMING_OFFSET +
	       slot * sizeof(struct kvm_exec_command_timing);
}

static struct kvm_exec_exit_timing *
kvm_exec_exit_timing(struct kvm_exec_executor *executor, u32 slot)
{
	struct kvm_exec_tsc_timing_header *header =
		kvm_exec_tsc_timing_header(executor);

	return (void *)header + KVM_EXEC_TSC_EXIT_TIMING_OFFSET +
	       slot * sizeof(struct kvm_exec_exit_timing);
}

static u64 kvm_exec_performance_ns(struct kvm_exec_executor *executor)
{
	return kvm_exec_tsc_timing_enabled(executor) ? 0 : ktime_get_ns();
}
...
static bool
kvm_exec_async_handoff_command(struct kvm_exec_executor *executor,
			       struct kvm_exec_capsule *source,
			       const struct kvm_exec_command *command)
{
	return source &&
	       (executor->domain->negotiated_features &
		KVM_EXEC_FEATURE_ASYNC_PIO_HANDOFF) &&
	       source->exit.async_request_pending &&
	       source->exit.async_executor_generation == executor->generation &&
		       ...
		       struct kvm_exec_capsule *capsule)
{
	...
	struct kvm_exec_exit_request request = {
		...
		.published_ns = kvm_exec_performance_ns(executor),
	...
	capsule->exit.async_executor_generation = executor->generation;
	if (kvm_exec_tsc_timing_enabled(executor)) {
		struct kvm_exec_exit_timing *timing =
			kvm_exec_exit_timing(
				executor,
				executor->exit_request_tail %
					KVM_EXEC_DISPATCH_RING_ENTRIES);
			       ...
			       u32 timing_slot)
{
	if (!capsule) {
		...
		executor->pending_entry_timing_slot = 0;
		return;
	}
	...
	executor->pending_entry_lifecycle_generation =
		capsule->lifecycle_generation;
	executor->pending_entry_timing_slot = timing_slot;
}
	...
	u32 timing_slot)
{
	if (!capsule) {
		...
		executor->pending_handoff_entry_timing_slot = 0;
		return;
	}
	...
	executor->pending_handoff_entry_lifecycle_generation =
		capsule->lifecycle_generation;
	executor->pending_handoff_entry_timing_slot = timing_slot;
}
...
kvm_exec_dispatch_record_entry(struct kvm_exec_executor *executor,
			       ...
			       u64 *request_sequence)
{
	struct kvm_exec_tsc_timing_header *timing_header = NULL;
	...
	if (!sequence && !handoff_sequence)
		return 0;
	entry_ns = kvm_exec_performance_ns(executor);
	if (kvm_exec_tsc_timing_enabled(executor)) {
		timing_header = kvm_exec_tsc_timing_header(executor);
		entry_tsc = kvm_arch_exec_host_tsc();
	}
	if (sequence &&
	    (executor->pending_entry_capsule_id != capsule->capsule_id ||
	     executor->pending_entry_lifecycle_generation !=
		     capsule->lifecycle_generation)) {
		kvm_exec_dispatch_expect_entry(executor, NULL, NULL, 0);
		sequence = 0;
	}
	if (sequence) {
		if (timing_header) {
			timing = kvm_exec_command_timing(
				executor, executor->pending_entry_timing_slot);
			if (READ_ONCE(timing->request_sequence) == sequence)
				WRITE_ONCE(timing->entry_attempt_tsc, entry_tsc);
			WRITE_ONCE(timing_header->last_entry_tsc, entry_tsc);
			/* Pairs with userspace's acquire load of the sequence. */
			smp_store_release(&timing_header->last_entry_sequence,
					  sequence);
		}
	...
	if (handoff_sequence &&
	    (executor->pending_handoff_entry_capsule_id != capsule->capsule_id ||
	     executor->pending_handoff_entry_lifecycle_generation !=
		     capsule->lifecycle_generation)) {
		kvm_exec_dispatch_expect_handoff_entry(executor, NULL, NULL, 0);
		handoff_sequence = 0;
	}
	if (handoff_sequence) {
		if (timing_header) {
			WRITE_ONCE(timing_header->last_handoff_entry_tsc,
				   entry_tsc);
			/* Pairs with userspace's acquire load of the sequence. */
			smp_store_release(
				&timing_header->last_handoff_entry_sequence,
				handoff_sequence);
		}
			  ...
			  struct kvm_exec_completion *completion)
{
	...
	struct kvm_exec_dispatch_header *header =
		kvm_exec_dispatch_header(executor);
	struct kvm_exec_command_timing *timing = NULL;
	struct kvm_exec_command command;
	...
	*completion = (struct kvm_exec_completion) {
		...
		.consumed_ns = kvm_exec_performance_ns(executor),
		.executor_return_count = executor->return_count,
	};
	if (kvm_exec_tsc_timing_enabled(executor)) {
		timing = kvm_exec_command_timing(executor, timing_slot);
		memset(timing, 0, sizeof(*timing));
		timing->request_sequence = command.request_sequence;
		timing->consumed_tsc = kvm_arch_exec_host_tsc();
	}
	...
	completion->applied_ns = kvm_exec_performance_ns(executor);
	if (timing)
		timing->applied_tsc = kvm_arch_exec_host_tsc();
	/*
				  ...
				  void __user *argp)
{
	...
	for (;;) {
		 ...
		 * userspace completion so recipient entry is not serialized behind
			  ...
			  void __user *argp)
{
	...
	if (kick.size != sizeof(kick) ||
	    ...
	    ((kick.flags & KVM_EXEC_KICK_F_RETURN_TO_VMM) &&
	     (kick.flags & KVM_EXEC_KICK_F_COMPLETION_AVAILABLE)) ||
	...
	if ((kick.flags & KVM_EXEC_KICK_F_RETURN_TO_VMM) &&
		...
		return -EOPNOTSUPP;
	if ((kick.flags & KVM_EXEC_KICK_F_COMPLETION_AVAILABLE) &&
	    !(domain->negotiated_features &
	      KVM_EXEC_FEATURE_ASYNC_PIO_HANDOFF))
		return -EOPNOTSUPP;
...
kvm_exec_validate_interrupt_locked(struct kvm_exec_executor *executor,
				   ...
				   struct kvm_exec_interrupt_outcome *outcome)
{
	struct kvm_exec_domain *domain = executor->domain;
	struct kvm_exec_capsule *capsule;
	...
	if (capsule->exit.async_request_pending ||
	    (capsule->exit.completion_pending &&
				 ...
				 u64 *accepted_ns, u64 *accepted_tsc)
{
	...
	if (request->request_sequence <= executor->last_interrupt_sequence) {
		...
		*accepted_ns = kvm_exec_performance_ns(executor);
	...
	struct kvm_exec_interrupt_outcome *outcome)
{
	...
	delivered_ns = kvm_exec_performance_ns(executor);
	delivered_tsc = kvm_arch_exec_host_tsc();
	outcome->publication_started_tsc = delivered_tsc;
	outcome->actual_delivery = KVM_EXEC_INTERRUPT_DELIVERY_DIRECT_KICK;
	epoch = atomic64_inc_return(&executor->kick_epoch);
	header = kvm_exec_dispatch_header(executor);
	WRITE_ONCE(header->kernel_kick_count, epoch);
	...
	struct kvm_exec_interrupt_outcome *outcome)
{
	...
	outcome->publication_started_tsc = delivered_tsc;
	outcome->actual_delivery = coalesced ?
			  ...
			  struct kvm_exec_interrupt_outcome *outcome)
{
	...
	if (!(domain->negotiated_features & KVM_EXEC_FEATURE_EXACT_INTERRUPT)) {
		kvm_exec_interrupt_outcome_finish(outcome, -EOPNOTSUPP);
		return -EOPNOTSUPP;
	}
	if (request->domain_generation != domain->generation ||
	    request->executor_generation != executor->generation) {
		kvm_exec_interrupt_outcome_finish(outcome, -ESTALE);
		return -ESTALE;
	}
	if (request->requested_delivery != domain->interrupt_delivery) {
		kvm_exec_interrupt_outcome_finish(outcome, -EINVAL);
		return -EINVAL;
	}
	...
	if (ret)
		goto out;
	outcome->accepted_tsc = accepted_tsc;
			      ...
			      bool stopped)
{
	...
	ret = kvm_exec_submit_interrupt(executor, &request, &outcome);
	if (!ret) {
		completion->actual_delivery = outcome.actual_delivery;
		completion->accepted_tsc = outcome.accepted_tsc;
		completion->delivered_tsc = outcome.publication_started_tsc;
	}
	kref_put(&executor->kref, kvm_exec_executor_free);
			       ...
			       void __user *argp)
{
	...
	return kvm_exec_submit_interrupt(executor, &request, &outcome);
}

static long kvm_exec_interrupt_result(struct kvm_exec_executor *executor,
				      void __user *argp)
{
	...
	if (result.size != sizeof(result) ||
	    result.flags & ~KVM_EXEC_INTERRUPT_F_RETAIN_HLT ||
	    !result.request_sequence || !result.capsule_id ||
	    !result.lifecycle_generation || result.vector > U8_MAX ||
	    !result.requested_delivery || result.result || result.status ||
	    result.actual_delivery || result.retain_halt_armed ||
	    result.accepted_tsc || result.publication_started_tsc ||
	    memchr_inv(result.reserved, 0, sizeof(result.reserved)))
		return -EINVAL;
	if (!(executor->domain->negotiated_features &
	      KVM_EXEC_FEATURE_INTERRUPT_RESULT))
		return -EOPNOTSUPP;
	...
	request = (struct kvm_exec_interrupt_request) {
		.domain_generation = result.domain_generation,
		.executor_generation = result.executor_generation,
	...
	kvm_exec_submit_interrupt(executor, &request, &outcome);
	result.result = outcome.result;
	result.status = outcome.status;
	result.actual_delivery = outcome.actual_delivery;
	result.retain_halt_armed = outcome.retain_halt_armed;
	result.accepted_tsc = outcome.accepted_tsc;
	result.publication_started_tsc = outcome.publication_started_tsc;
				    ...
				    unsigned long arg)
{
	...
	if (ioctl == KVM_EXEC_INTERRUPT_RESULT)
		return kvm_exec_interrupt_result(executor, (void __user *)arg);
				  ...
				  struct vm_area_struct *vma)
{
	...
	if (!executor->dispatch_region)
		return -EOPNOTSUPP;
	if (vma->vm_pgoff ||
	    size != kvm_exec_dispatch_region_size(executor) ||
	    !(vma->vm_flags & VM_SHARED) || (vma->vm_flags & VM_EXEC))
		return -EINVAL;
...
int kvm_dev_ioctl_create_exec_domain(void __user *argp)
{
	...
	if (create.size != sizeof(create) || create.flags ||
	    ...
	    ((create.requested_features &
	      KVM_EXEC_FEATURE_HANDOFF_ENTRY_OBSERVATION) &&
	     !(create.requested_features & KVM_EXEC_FEATURE_DYNAMIC_DISPATCH)) ||
	    ((create.requested_features & KVM_EXEC_FEATURE_TSC_TIMING) &&
	     !(create.requested_features & KVM_EXEC_FEATURE_DYNAMIC_DISPATCH)) ||
	    ((create.requested_features & KVM_EXEC_FEATURE_RETURN_KICK) &&
	     !(create.requested_features & KVM_EXEC_FEATURE_DYNAMIC_DISPATCH)) ||
	    ((create.requested_features & KVM_EXEC_FEATURE_EXACT_INTERRUPT) &&
	     !(create.requested_features & KVM_EXEC_FEATURE_DYNAMIC_DISPATCH)) ||
	    ((create.requested_features & KVM_EXEC_FEATURE_INTERRUPT_RESULT) &&
	     !(create.requested_features & KVM_EXEC_FEATURE_EXACT_INTERRUPT)) ||
	    ((create.requested_features &
	      KVM_EXEC_FEATURE_INTERRUPT_PUBLICATION) &&
	     !(create.requested_features & KVM_EXEC_FEATURE_EXACT_INTERRUPT)) ||
